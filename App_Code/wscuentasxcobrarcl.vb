...
Public Class wscuentasxcobrarcl
    ...
    Public Function consultar() As List(Of [Datos])
        ...
        Dim StrEncabezado As String = "SELECT " &
                                                ...
                                                "FROM ENC_FACTURA ef WHERE ef.Id_Clt = c.Id_Clt  and ef.valorCredito > 0 and ef.estado = 1 ) filtrar " &
                                                ...
                                                "WHERE ef2.id_enc = ef.id_enc  and ef2.estado = 1 " &
                                                ")agrupar), 0)notasCredito, id_enc " &
                                                "FROM ENC_FACTURA ef WHERE ef.Id_Clt = c.Id_Clt and ef.estado = 1  and ef.valorCredito > 0 AND ef.fecha > '2015-11-01') filtrar " &
        ...
        For Each row As DataRow In TablaEncabezado.Rows
            ...
            If Convert.ToDouble(row(1).ToString) <> 0 Or Convert.ToDouble(row(2).ToString) Or Convert.ToDouble(row(3).ToString) <> 0 Or Convert.ToDouble(row(4).ToString) <> 0 Then
    ...
    Public Function getDetalle(ByVal sucursal As Integer, ByVal dias As Integer) As List(Of [Datos])
        ...
        Dim strBusqueda As String = "SELECT firma, Nom_clt cliente, CONVERT(varchar(10), Fecha, 103) fecha, SUM(factura - recibos - notasCredito), dias, '' paciente FROM (SELECT " &
                                    ...
                                    "WHERE ef.Id_Clt = " & sucursal & "  and ef.valorCredito > 0 and ef.estado =1) filtrar " &
