    ...
    $('#btn-Cliente').click(function () {
        ...
        if (nit == "" && descripcion == "" && direccion == "") {
            ...
            data1 = '{ nit_clt : "' + nit + '",  Nom_clt : "' + descripcion +
                ...
                ',  Limite_Credito : ' + limite + ',  Dias_Credito : ' + dia + ',  Correo_Clt : "' + correo + '",  Observ_Clt : "' + observacion + '",  precio: ' + precio +'}';
                ...
                success: function (msg) {
                    ...
                    $.each(msg.d, function () {
                        var tds = "<tr class='odd'><td>" + this.codigo + "</td><td>" + this.descripcion + "</td><td>" + this.cantidad + "</td>" +
                            "<td><span data-dismiss='modal' onclick='cargarProducto(" + this.id + ",\"" + this.codigo + "\",\"" + this.descripcion + "\",\"" + this.cantidad + "\"," + this.precio + "," + this.tipo + "," + this.tipoArt+")' class='btn btn-sm btn-outline-info' data-container='body' data-trigger='hover' data-toggle='popover' data-placement='bottom' data-content='AGREGAR AL CARRITO DE COMPRAS' data-original-title='' title ='' > " +
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                        if (msg.d.length == 0) {
                            ...
                            cargarProducto(msg.d[0].id, msg.d[0].codigo, msg.d[0].descripcion, msg.d[0].cantidad, msg.d[0].precio, msg.d[0].tipo, msg.d[0].tipoArt);
                        }
                    }
                });
            }
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if (arr[0] == 'SUCCESS') {
                    ...
                    cargarProducto(id, codigo, descripcion, cantidad, preci,1);
                } else {
                    $('.jq-toast-wrap').remove();
                    $.toast({
                        heading: '¡ERROR!',
...
function cargarProducto(id, codigo, descripcion, cantidad, precio, tipo, tipoArt) {
    ...
    if ($('#cantidad').val() == "") {
        ...
        if (!existe) {
            var linea = { 'cantidad': $('#cantidad').val(), 'codigo': codigo, 'descripcion': descripcion, 'id': id, 'precio': $('#precio').val(), 'bodega': $('#bodega').val(), 'bo': $('#bodega option:selected').text(), 'tipoArt': tipoArt  };
            datos.push(linea);
